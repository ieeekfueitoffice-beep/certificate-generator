...
// Finds (or creates, on first use) a per-event subfolder inside the app's
// root Drive folder, so each event's template + certificates are grouped
// together under a folder named after that event.
async function ensureTaskFolder(user, task) {
  if (task.drive_folder_id) return task.drive_folder_id;

  const drive = await getDriveClient(user);
  const parentId = await ensureAppFolder(user);
  const folderName = `${task.title} (${task.id.slice(0, 6)})`.replace(/[\\/]/g, "-");
  ...
  db.prepare("UPDATE tasks SET drive_folder_id = ? WHERE id = ?").run(folderId, task.id);
  task.drive_folder_id = folderId;
  return folderId;
}

// Uploads a buffer to the user's Drive app folder, returns the new file's ID.
// Pass a folderId (e.g. from ensureTaskFolder) to place it in a specific
// event's subfolder instead of the root app folder.
async function uploadFile(user, { buffer, filename, mimeType, folderId }) {
  const drive = await getDriveClient(user);
  const targetFolder = folderId || (await ensureAppFolder(user));
...
module.exports = { uploadFile, downloadFile, deleteFile, ensureAppFolder, ensureTaskFolder };
